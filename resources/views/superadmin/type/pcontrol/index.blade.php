  ...
  <div class="col-md-12">
      <div class="card">
          <table id="myTable" class="table table-hover table-borderless table-responsive">
              ...
              <tbody>
                  @foreach ($pest_controls as $key => $pest_control)
                  <tr>
                      <td scope="row">{{ $key+1 }}</td>
                      <td>{{ $pest_control->foreman }}</td>
                      <td>{{ $pest_control->planting_year }}</td>
                      <td>{{ $pest_control->date }}</td>
                      <td>{{ $pest_control->total_coverage }}</td>
                      <td>{{ $pest_control->available_coverage }}</td>
                      <td>{{ $pest_control->population_coverage }}</td>
                      <td>{{ $pest_control->population_perblock }}</td>
                      <td>{{ $pest_control->block }}</td>
                      <td>{{ $pest_control->subforeman }}</td>
                      <td>{{ $pest_control->job_type }}</td>
                      <td>
