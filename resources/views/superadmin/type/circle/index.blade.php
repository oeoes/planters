...
<div class="card col-md-12">
    <table id="myTable" class="table" style="font-size:11pt">
        ...
        <tbody>
            ...
            @php $reference = App\Models\BlockReference::find($circle->block_ref_id); @endphp
            <tr>
                <td scope="row">{{ $key+1 }}</td>
                <td>{{ date('d-m-Y', strtotime($circle->date))}}</td>
                <td>{{ foreman($circle->foreman_id)->name }}</td>
                <td>{{ $reference->total_coverage }} Ha</td>
                <td>{{ $reference->population_coverage }} Ha</td>
                <td>{{ $reference->population_perblock }}</td>
                <td>{{ subforeman($circle->subforeman_id)->name }}</td>
                <td>{{ jobtype($reference->jobtype_id) }}</td>
                <td>
                    <a href="{{ route('superadmin.circle.detail', [$circle->block_ref_id, $circle->id]) }}" class="btn btn-default btn-sm">
